#!/bin/bash
# 检查数据库闪回恢复区使用情况
# 通过sys用户查询：select sum(t.PERCENT_SPACE_USED) from V$FLASH_RECOVERY_AREA_USAGE t;
...
SQLFILE=${datadir}/check_ora_rac_archivelog.sql
OUTFILE=${datadir}/check_ora_rac_archivelog.txt
echo -n "
select sum(t.PERCENT_SPACE_USED) from V\$FLASH_RECOVERY_AREA_USAGE t;
" > $SQLFILE

SQLFILEa=${datadir}/check_ora_rac_archivelog_a.sql
echo -n "
